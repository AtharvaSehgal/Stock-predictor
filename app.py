...
st.sidebar.markdown("Powered by LSTM + Llama3")

# ---- Full NSE stock list ----
@st.cache_data(ttl=86400)  # Cache for 24 hours
def load_nse_symbols():
    """Load all NSE listed stocks dynamically."""
    import requests
    try:
        # NSE India's official stock list
        url = "https://archives.nseindia.com/content/equities/EQUITY_L.csv"
        df_nse = pd.read_csv(url)
        # Returns NAME OF COMPANY + SYMBOL columns
        stocks = {}
        ...
        # Fallback to popular stocks if NSE is unreachable
        return {
...
with st.sidebar:
    ...
    # Create reverse mapping for looking up name by symbol
    ...
    if not filtered:
        ...
        filtered = all_stocks  # fallback show all

    # Get current display name
    current_name = symbol_to_name.get(st.session_state.selected_symbol, list(filtered.keys())[0] if filtered else list(all_stocks.keys())[0])
    
    # Make sure current name is in filtered list, else pick first
    if current_name not in filtered:
        current_name = list(filtered.keys())[0] if filtered else list(all_stocks.keys())[0]
    
    # Selectbox - update session state when changed
    selected_name = st.selectbox(
        f"Select ({len(filtered)} results)",
        list(filtered.keys()),
        index=list(filtered.keys()).index(current_name) if current_name in filtered else 0,
        key="stock_select"
    )
    st.session_state.selected_symbol = filtered[selected_name]

    # Direct symbol entry - optional override
    ...
    if custom_symbol:
        if not custom_symbol.endswith('.NS') and not custom_symbol.endswith('.BO'):
            custom_symbol = custom_symbol + '.NS'
...
period = st.sidebar.slider("Historical data (days)", 90, 730, 365)
forecast_days = st.sidebar.slider("Forecast days ahead", 1, 14, 5)
train_model_btn = st.sidebar.button("🧠 Train / Retrain LSTM")

st.sidebar.markdown("---")
st.sidebar.markdown("⚠️ For educational use only. Not financial advice.")
...
with col_tech:
    ...
    emoji = signal_color.get(signals['overall_signal'], '⚪')
    ...
    fig_rsi = go.Figure(go.Indicator(
        ...
        gauge={
            ...
            'threshold': {'line': {'color': "red", 'width': 4}, 'value': signals['RSI']}
        }
    ))
...
with col_sent:
    ...
    sent_col1.metric("Positive", sentiment['positive_count'])
    sent_col2.metric("Neutral", sentiment['neutral_count'])
    sent_col3.metric("Negative", sentiment['negative_count'])
...
if train_model_btn:
    with st.spinner(f"Training LSTM on {len(df)} days of data... (2-3 mins on M4)"):
        model, scaler, features, metrics = train_model(symbol, df, epochs=50)
        st.session_state[f'model_{symbol}'] = (model, scaler, features)
        st.success(f"✅ Model trained! Accuracy: ~{metrics['accuracy']:.1f}% | MAE: ₹{metrics['mae']:.2f}")
...
if f'model_{symbol}' in st.session_state:
    ...
    if prediction:
        ...
        with pred_col:
            ...
            fig_pred = px.line(pred_df, x='Day', y='Predicted Price (₹)',
                               markers=True, title=f"{symbol} — {forecast_days}-Day Forecast")
            fig_pred.update_traces(line_color='#00CC96', line_width=2.5)
            st.plotly_chart(fig_pred, use_container_width=True)
else:
    st.info("👆 Click **Train / Retrain LSTM** in the sidebar to generate price predictions.")

# ---- AI Insights ----
st.markdown("---")
st.subheader("🧠 AI Analysis (Llama3)")

if st.button("Generate AI Insights"):
    prediction_data = st.session_state.get(f'model_{symbol}', None)
    pred_result = None
    if prediction_data:
        model, scaler, features = prediction_data
        pred_result = predict_next_days(symbol, df, scaler, features, days_ahead=5)

    with st.spinner("Llama3 is analyzing all signals..."):
...
if f'model_{symbol}' in st.session_state:
    ...
    if st.button("📊 Run Full Backtest"):
        ...
        **Directional Accuracy: {dir_acc}% — {grade}**
        - Above 50% means the model is better than random guessing
        - Above 60% is considered good for stock prediction
        - Above 65% is excellent — hedge funds aim for this
        
        **MAE of ₹{m['MAE']}** means on average the predicted price 
        is ₹{m['MAE']} away from the actual price.
    ...
    st.info("👆 Train the LSTM model first using the sidebar button, then run the backtest.")